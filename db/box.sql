...
, passwords                 varchar(200) NOT NULL                               comment '用户设定的密码'
...
, frozen                    tinyint(1) NOT NULL DEFAULT 0                       comment '账户是否被冻结'
, attempts                   tinyint(1) NOT NULL DEFAULT 0                      comment '用户尝试密码次数'
, frozenTo                  timestamp NULL DEFAULT NULL                         comment '账户解冻时间'
...
, branchID                  int(10) NOT NULL DEFAULT 1                          comment '所属部门ID'
...
, transBoxHash              varchar(100) NOT NULL UNIQUE                             comment 'transBox上链的哈希值'
...
, reason                    varchar(200) NULL                                         comment '审批拒绝或撤回申请原因'
...
, singleLimit               varchar(20) NOT NULL DEFAULT 0                            comment '单笔转账限额'
...
CREATE TABLE `tb_deposit_history` (
  id                        bigint(20)  AUTO_INCREMENT PRIMARY KEY                    comment '充值记录ID，主键'
...
, updatedAt                 timestamp NULL DEFAULT CURRENT_TIMESTAMP                  comment '充值时间'
, UNIQUE INDEX uniquetx (`fromAddr`,`toAddr`,`txID`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
...
CREATE TABLE `tb_department` (
  id                        int(10)  AUTO_INCREMENT PRIMARY KEY                    	comment '部门ID，自增'
, name                  	  varchar(20) NOT NULL                                    comment '部门名称'
, bIndex                    int(10) NOT NULL                                      	comment '部门排序索引'
, createdBy                 int(10) NOT NULL                                        comment '创建者账号ID'
,	createdAt                 timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP        		comment '申请创建时间'
, updatedAt                 timestamp NULL DEFAULT NULL
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
#更新tb_department的 updatedAt
DROP trigger IF EXISTS trg_department_update;
CREATE TRIGGER `trg_department_update` BEFORE UPDATE ON `tb_department` FOR EACH ROW set new.updatedAt=CURRENT_TIMESTAMP;
BEGIN;
INSERT INTO `tb_department` VALUES (1, '其他', 1, 0, CURRENT_TIMESTAMP, null);
COMMIT;

#审批流分币种额度
DROP TABLE IF EXISTS `tb_flow_limit`;
CREATE TABLE `tb_flow_limit` (
  flowID                    bigint(20) NOT NULL                     	              comment '审批流ID'
, currencyID                int(10) NOT NULL                                        comment '币种ID'
, frozen                    tinyint(1) NOT NULL DEFAULT 0                           comment '额度是否被用完'
, frozenTo                  timestamp NULL                                          comment '解冻时间'
,	amountLeft                varchar(20) NOT NULL DEFAULT 0        		              comment '剩余额度'
, amount                    varchar(20) NOT NULL DEFAULT 0                          comment '设定的额度'
, period                    int(10) NOT NULL DEFAULT 0                              comment '设定的额度恢复时间'
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci;
